...
def test_exec(config):
    ...
    if 'omap' in default_data_log:
        log.info("omap is the default data log backing in nautilus")
    else:
        log.info("fifo is the default data log backing in pacific and above")

    # change the default datalog backing to FIFO
    if config.test_ops["change_datalog_backing"] is True:
        if default_data_log == 'omap':
            log.info('change the default logtype from omap to fifo')
            cmd = ('radosgw-admin datalog type --log-type=fifo')
