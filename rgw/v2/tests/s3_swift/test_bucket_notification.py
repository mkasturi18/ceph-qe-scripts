...
#from v2.tests.s3_swift.reusables.notification import notification
        ...
        if config.test_ops["create_bucket"] is True:
            ...
            for bc in range(config.bucket_count):
                ...
                if config.test_ops["create_topic"] is True:
                    ...
                    persistent = False
                    topic_name = 'cephci-kafka-'+ack_type+'-ack-type'+str(random.randrange(1, 1000))
                    ...
                    topic = reusable.create_topic(rgw_sns_conn,endpoint,ack_type,topic_name,persistent)
                ...
                if config.test_ops.get("copy_object", False):
                    ...
                    status = rgw_s3_client.copy_object(Bucket= bucket_name_to_create, Key = "copy_of_object",CopySource={"Bucket": bucket_name_to_create, "Key": s3_object_name})
                    if status is None:
            ...
            event_record_path = '../../test_data/event_record'
            reusable.start_kafka_broker_consumer(topic_name,event_record_path)

            #stop kafka and zookeeper services
            reusable.stop_kafka_broker() 
            ...
            verify = reusable.verify_event_record(event,bucket_name_to_create,event_record_path)
            if verify is False:
