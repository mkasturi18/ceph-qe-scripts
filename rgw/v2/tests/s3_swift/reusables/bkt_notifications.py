...
#    path = utils.exec_shell_cmd("echo $KAFKA_HOME)" #'/home/cephuser/kafka*/'
    ...
    if "nautilus" in ceph_version:
        cmd = f"sudo {KAFKA_HOME}/bin/kafka-console-consumer.sh --bootstrap-server kafka://10.0.211.77:9092 --from-beginning --topic {topic_name} --timeout-ms 30000 >> {event_record_path}"
    else:
        cmd = f"sudo {KAFKA_HOME}/bin/kafka-console-consumer.sh --bootstrap-server kafka://10.0.211.77:9092 --from-beginning --topic {topic_name} --zookeeper localhost:2181 --timeout-ms 30000 >> {event_record_path}"
